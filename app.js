...
// Base bumps on hand-edits to venues-data.js; the extracted-layer stamp is
// appended so every pipeline re-sync reseeds cached clients automatically.
const SEED_VERSION =
  "2026-vancouver-40-discovery-v1" +
  (typeof EXTRACTED_DATA_VERSION === "string" ? ":" + EXTRACTED_DATA_VERSION : "");
...
function sampleVenues() {
  ...
  // pipeline/writeback.js) over the seed. Extracted deals apply only where
  // the automated read corroborated the hand-verified schedule; the schedule
  // itself and every other field stay from the seed. Assembled from the raw
