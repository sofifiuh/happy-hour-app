...
function sampleVenues() {
  const seed = JSON.parse(JSON.stringify(VENUES_SEED));
  // Merge the machine-written layer (venues-extracted.js, generated by
  // pipeline/writeback.js). Extracted deals apply only where the automated
  // read corroborated the hand-verified schedule; the schedule itself and
  // every other field stay from the seed.
  if (typeof VENUES_EXTRACTED === "object" && VENUES_EXTRACTED) {
    for (const v of seed) {
      const x = VENUES_EXTRACTED[v.id];
      if (x?.deals?.length) {
        v.happy_hour.deals = JSON.parse(JSON.stringify(x.deals));
        v.happy_hour.deals_source = { url: x.source_url, extracted_at: x.extracted_at };
      }
    }
  }
  if (typeof VENUES_DISCOVERED !== "undefined" && Array.isArray(VENUES_DISCOVERED)) {
    seed.push(...JSON.parse(JSON.stringify(VENUES_DISCOVERED)));
  }
  return seed;
}
